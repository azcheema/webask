...
| 2 — Services / Locations / Industries    | **Part done**   | 9 service pages + 3 industry pages + the service catalogue (`data/services.ts` 57% → 0%, `data/copy/pricing.ts` 82% → 0%); FAQ ordering pricing-first on all 9. **All 3 location hubs written 2026-07-29** — 0.0% against the US fork hubs, 1.1% cross-page — with nav, sitemap, a11y, Lighthouse and a JSON-LD `areaServed`/D1 assertion wired. ⚠️ Still forked: `data/copy/free-audit.ts` 39% (**still no verdict**, though four implied-client claims in it were fixed 2026-07-29) and `data/copy/legal.ts` 69% (still says "Naxdor … enskild firma"; gated behind D3). `data/copy/process.ts` 61% is duplicate **by decision** (docs/01). Remaining: programmatic batch, link mesh beyond the hubs, citations, and the **industry-page copy-review checklist** |
