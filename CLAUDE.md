...
> which either pass or do not". Found in `data/services.ts`, `data/copy/process.ts`,
> `data/copy/home.ts` and `content/services/maintenance-support.mdx`. Before writing any
> performance claim, open `.lighthouserc.cjs` and check which side of `error`/`warn` it
> sits on.
...
| 2 — Services / Locations / Industries    | **Part done**   | 9 service pages + 3 industry pages written for the UK; FAQ ordering now pricing-first on all 9 (doc 08 § 7). **`data/services.ts` (57% → 0%) and `data/copy/pricing.ts` (82% → 0%) now rewritten too** — the catalogue that renders around the MDX bodies. ⚠️ Still forked: `data/copy/free-audit.ts` 43% (no verdict recorded) and `data/copy/legal.ts` 85% (still says "Naxdor … enskild firma"; gated behind D3). `data/copy/process.ts` 67% is duplicate **by decision** (docs/01). Remaining: 3 location hubs, programmatic batch, link mesh, nav/footer, citations |
