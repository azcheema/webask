...
| `00-decisions-and-open-questions.md`                                     | What was decided, what is assumed, what this re-opens, what must still be decided                | 🟡 Q-list seeded, owners pending; decision log 7 rows (S2: the public rate file supersedes the in-account export — ❓ founder to confirm; the eject commission forces D11)                                                                                                                                                                     | 2026-09-24   |
...
| `03-uk-demand-and-keywords.md` + `keywords-draft.json` + `serp-log.json` | Does UK demand exist per service, at what evidence tier, which terms each page owns              | 🟡 S3 part done: 49 `google.co.uk` + 30 Bing + 154 suggest captures in `serp-log.json`; §§ 6–9 written for text-back, reviews, receptionist and email/SMS on Google evidence, the other pages on Bing only; `keywords-draft.json` 79 rows, 0 duplicates; ~56 Google captures still queued (`03` § 6.11) — Google blocked twice (429, then 403) | 2026-09-24   |
...
**Next:** finish S3 — re-run the ~56 queued `google.co.uk` captures listed in `03` § 6.11 (landing
pages, the profile service, the bundle and GBP-cost cluster, local SEO, the fold tests, the vertical,
near-me and comparison terms) at a slower pace and from a fresh browser context, then complete
`03` §§ 6–9 for those pages and lift the "Bing-only" markers; then S4 (`04` Set C from the `03`
head-term SERPs; `06` verticals with the CMA Order and the DBT response read first). S2 closed on
24 September 2026 with `01` 🟢; S3 ran the same day until Google blocked the session twice (HTTP 429
after 45 captures at 4–8 s, HTTP 403 after 5 more at 12–18 s) — both are records in `serp-log.json`.
The LC Phone UK rates were found on the guide's **public** attachments and are in `01` § 4.1; the
founder's in-account action is reduced to a wallet-line spot-check before S5 closes (`00` § 6, ❓ to
confirm).
