...
| `03-uk-demand-and-keywords.md` + `keywords-draft.json` + `serp-log.json` | Does UK demand exist per service, at what evidence tier, which terms each page owns              | 🟡 S3 part done: 59 `google.co.uk` + 32 Bing + 154 suggest captures in `serp-log.json`; §§ 6–9 written on Google evidence for text-back, reviews, receptionist, email/SMS, landing pages and the head terms of the profile, bundle and local-SEO pages, the fold tests and verticals on Bing only; `keywords-draft.json` 84 rows, 0 duplicates; 48 Google captures still queued (`03` § 6.11, five to a window) — Google blocked three times on 24 September (429, 403, 403), then a five-capture window held; Q30 | 2026-09-24   |
