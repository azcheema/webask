...
- [x] 3 industry pages, each run through the **copy-review checklist** (doc 03 § B4) —
      2026-07-29, `feature/industry-copy-review`. First time the checklist had ever been run;
      the pages had been written, reported done and then had live defects found twice by
      passes that were looking for something else. **157 findings → 104 unique (13 blockers)**,
      then a re-check found **61 more, 42 of them introduced by the corrections themselves**.
      The blockers: a **prescription-only medicine named in sales copy** and in `FAQPage`
      JSON-LD, on the page whose blockquote boasts it names none; **"remotely across the US"**
      and **"the metros we focus on"** rendering on 12 pages; portfolio-composition and
      attach-rate claims on a firm with no clients; **review-gating advice on the page selling
      review compliance**; an invented "CQC display duty"; and a beauty-wellness regulatory
      argument that traced to **no line in the bundle** — doc 03 had no section for that
      vertical at all. Three fixes went wider than the pages, because the same defect was live
      elsewhere and stopping at the page boundary is what caused this in the first place:
...
| 2 — Services / Locations / Industries | **Part done**   | 9 service pages + 3 industry pages + the service catalogue (2026-07-28), all 3 location hubs (2026-07-29), and the **doc 03 § B4 copy-review checklist finally run over the three industry pages** (2026-07-29) — 218 findings across two rounds, 13 blockers, incl. a POM named in `FAQPage` JSON-LD and two US template strings live on 12 pages. Remaining: first programmatic batch, `CrossLinkGrid` mesh beyond the hubs, citations |
